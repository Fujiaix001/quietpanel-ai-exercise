# QuietPanel v6.8.8-test-nodistribute
...
相簿頁只在畫面停留於第三頁時輪播，每張照片停留時間可在相簿設定中調整為 10～300 秒。比例與螢幕不同的照片會保持滿版，沿著超出畫面的方向做非常緩慢的微幅平移；每張照片只移動完整可平移距離的 20%，不強求在單次播放中呈現所有裁切區域。平移約以 15fps 更新，在維持慢速動態質感的同時降低舊手機的持續運算負擔；換圖時以較慢的淡出與淡入銜接。進入相簿頁會隱藏 QuietPanel 標題列與頁面指示；大型雙行時鐘顯示時間、日期與星期，不顯示秒。可單指拖曳時鐘日期區塊到任意位置，或用雙指縮放讓兩行維持比例一起放大、縮小（75%～250%，會保存）。相簿設定可選擇是否顯示半透明底板與 12 種 Android 系統字型：預設、等寬、襯線、細體、濃黑、窄體、中黑、手寫、數位鐘、時鐘、斜體、小米；舊手機缺少個別字型時會自動使用最接近的系統替代字型。程式一次只解碼一張圖片以降低舊手機記憶體用量。相簿設定支援多個目錄及其子目錄，最多收集 10,000 張照片；若尚未選擇，仍沿用外接 SD 卡 `/sdcard/QuietPanel/Photos`。離開相簿頁時輪播、平移與時鐘更新會暫停並釋放相簿圖片。相簿完全由手機本機處理，不經 Windows Bridge 傳輸。

v6.8.8-test-nodistribute 目前提供六種相簿時鐘字型：預設、濃黑、時鐘、手寫、Storopia、Oxanium；「時鐘」與 Storopia 都使用完整 Storopia 字型顯示時間與英文日期，Oxanium 顯示英文日期，其餘字型維持中文日期。Storopia 字型檔因授權未確認而僅保留於本機並由 Git 忽略；Oxanium 與 OFL 授權檔可公開追蹤。此版本僅供測試，請勿分發。
