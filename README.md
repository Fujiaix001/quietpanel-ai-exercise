...
3. 相簿：隨機輪播手機 `/sdcard/QuietPanel/Photos` 中的 JPG、JPEG 與 PNG，右下角顯示日期時間。
...
相簿頁只在畫面停留於第三頁時輪播，每 30 秒隨機切換一張，一次只解碼一張圖片以降低舊手機記憶體用量。請事先將照片縮小後放入 `/sdcard/QuietPanel/Photos`；離開相簿頁時輪播與時鐘更新會暫停。相簿完全由手機本機處理，不經 Windows Bridge 傳輸。
