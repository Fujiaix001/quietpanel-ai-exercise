...
本次結案時，正式穩定基線為 **v6.5.0（ADB 方案）**；後續新增的第三頁設定功能已編為 **v6.6.0**，並以 **v6.6.1** 修正拖曳換頁與設定頁滾動問題，再以 **v6.6.2** 修正時鐘預設位置與設定按鈕返回後未隱藏問題。**v6.7.0** 新增四種系統內建時鐘字體選項；**v6.8.0** 再擴充為 12 種系統字型，並加入可保存的雙指縮放時鐘與日期。兩版都未增加第三方依賴或高成本視覺效果。ADB 仍是目前這台舊手機最可靠、資源最節省的使用方式。
