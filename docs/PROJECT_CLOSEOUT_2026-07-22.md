...
本次結案時，正式穩定基線為 **v6.5.0（ADB 方案）**；後續新增的第三頁設定功能已編為 **v6.6.0**，並以 **v6.6.1** 修正拖曳換頁與設定頁滾動問題。手機已安裝 `versionName=6.6.1`、`versionCode=661`，PC 端 Bridge 已可透過 ADB 運作。ADB 仍是目前這台舊手機最可靠、資源最節省的使用方式。
