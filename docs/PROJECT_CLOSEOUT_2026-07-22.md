...
本次結案時，正式穩定基線為 **v6.5.0（ADB 方案）**；後續新增的第三頁設定功能已編為 **v6.6.0**，並以 **v6.6.1** 修正拖曳換頁與設定頁滾動問題，再以 **v6.6.2** 修正時鐘預設位置與設定按鈕返回後未隱藏問題。**v6.7.0** 新增四種系統內建時鐘字體選項，未增加第三方依賴或高成本視覺效果。手機已安裝 `versionName=6.7.0`、`versionCode=670`，PC 端 Bridge 已可透過 ADB 運作。ADB 仍是目前這台舊手機最可靠、資源最節省的使用方式。
...
3. **相簿時鐘**：手機選擇的一個或多個 SD 卡資料夾（含子資料夾）中的 JPG／JPEG／PNG。滿版裁切、淡入淡出、極慢微幅平移、大型雙行時間與日期；不顯示秒。點一下相簿畫面才出現「設定」按鈕，可調整照片停留 10～300 秒、半透明底板，以及拖曳並保存時鐘位置。預設相容路徑為 `/sdcard/QuietPanel/Photos`。
