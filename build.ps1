...
# 1. Rust Bridge Tests & Release Build (v8.1.1 Wi-Fi + Bluetooth Dual Mode)
...
try {
    ...
    Write-Output "Building Rust Bridge v8.1.1 (Wi-Fi + Bluetooth Dual Mode)..."
    cargo build --release
...
try {
    Write-Output "Building Android APK v8.1.1..."
...
Copy-IfDifferent -Source $wifiExeTemp -Destination (Join-Path $dist 'QuietPanelBridge-v8.1.1.exe')
...
Copy-Item -LiteralPath $builtApk -Destination (Join-Path $dist 'QuietPanel-v8.1.1.apk') -Force
