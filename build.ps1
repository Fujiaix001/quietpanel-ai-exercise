...
# 1. Rust Bridge Tests & ADB Release Build (v6.8.16.3-test)
...
try {
    ...
    Write-Output "Building Rust Bridge v6.8.16.3-test (USB ADB Mode)..."
...
try {
    Write-Output "Building Android APK v6.8.16.3-test..."
...
Copy-IfDifferent -Source $adbExeTemp -Destination (Join-Path $dist 'QuietPanelBridge-v6.8.16.3-test-ADB.exe')

$builtApk = Join-Path $projectRoot 'android\app\build\outputs\apk\release\app-release.apk'
Copy-Item -LiteralPath $builtApk -Destination (Join-Path $dist 'QuietPanel-v6.8.16.3-test-ADB.apk') -Force
...
Write-Output "Successfully built QuietPanel v6.8.16.3-test (USB ADB Mode)!"
